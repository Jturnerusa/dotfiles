...
class CPU:
	...
	@staticmethod
	def coretemp():
		return str(round(psutil.sensors_temperatures(fahrenheit=False)["k10temp"][0][1], 1))
...
while True:
	sys.stdout.write(f"Network: [UP:{Network.get_bytes_up()}KB/s DOWN:{Network.get_bytes_down()}KB/s] | CPU: [{CPU.freq()} GHz] [{CPU.coretemp()} C] | CRYPTROOT: [{DISK.disk_total('/')} GB/{DISK.disk_used('/')} GB] | CRYPTSSD: [{DISK.disk_total('/mnt/ssd/')} GB/{DISK.disk_used('/mnt/ssd/')} GB] | RAM: [{RAM.ram_total()} GB/{RAM.ram_used()} GB] | SWAP: [{RAM.swap_total()} GB/{RAM.swap_used()} GB] | TIME: [{Time.date()}] ")
	sys.stdout.flush()
	sleep(.75)
